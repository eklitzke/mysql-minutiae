...
\begin{frame}{MySQL Replication Mechanics}
  ...
  \begin{itemize}
  \item \texttt{IO\_THREAD}: Connects to master, writes sql statements to the relay logs
  \item \texttt{SQL\_THREAD}: Reads the relay logs, executes statements in them
  \end{itemize}
\end{frame}
...
\begin{frame}{IO\_THREAD}
  ...
  \begin{itemize}
  \item \texttt{Master\_Log\_File}: binlog file on the master being read from
  \item \texttt{Read\_Master\_Log\_Pos}: offset into binlog file on master being read from
  \end{itemize}
\end{frame}
...
\begin{frame}{SQL\_THREAD}
  ...
  \begin{itemize}
  \item \texttt{Relay\_Log\_File}: relay logfile on the slave where statements are being executed
  \item \texttt{Relay\_Log\_Pos}: offset into relay logfile on slave
  \item \texttt{Relay\_Master\_Log\_File}: logfile on master that current relay position corresponds to
  \item \texttt{Exec\_Master\_Log\_File}: position of logfile on master that current position corresponds to
  \item \texttt{Seconds\_Behind\_Master}: difference in time between the
