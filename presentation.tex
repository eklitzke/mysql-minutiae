...
\end{frame}
\begin{frame}{Transaction Isolation Levels}
  In the previous example, a transaction isolation like \texttt{READ
    UNCOMMITTED} would make it possible for a second transaction to see just one
  of the inserts, but not the other.
  \newline
  \newline
  A higher transaction isolation level like
  \texttt{READ COMMITTED} would make it possible only to see rows that have been
  fully committed.
\end{frame}
